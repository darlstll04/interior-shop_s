...
export const Kitchen = () => {
  ...
  const KITCHEN_URL = "http://localhost:3800/kitchen";
  ...
  const getKitchen = async () => {
    try {
      const { data } = await axios(KITCHEN_URL);
      setKitchen(data);
    } catch (err) {
      ...
      {!error && (
        <div className="container">
          ...
          <div className={s.cards}>
            {kitchen.map((product) => (
              <KitchenProduct key={product.id} product={product} addToOrder={()=> addToOrder(product)}/>
              ))}
          </div>
          </div>
      )}
